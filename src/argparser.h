...
#define ARGPARSER_TMPL template <typename T, int nargs, bool required>
...
public:
  typedef struct dscrpt {
    ...
    dscrpt(const std::string &name, const std::string &help,
           const std::string &flag, bool required)
...
private:
  ...
  private:
    ...
    proxy(const basic_argument *entry, int idx) : entry_(entry), idx_(idx) {}
...
*/
template <typename T, int nargs, class enable = void> struct is_valid_argument {
  static const bool value = false;
};

template <> struct is_valid_argument<bool, 0> {
...
template <> struct parser<char *> {
  static char *parse(char *s) { return s; }
};

template <> struct parser<std::string> {
  static std::string parse(char *s) { return std::string(s); }
};

template <typename T>
struct parser<T, typename std::enable_if<std::is_integral<T>::value>::type> {
  static T parse(char *s) { return static_cast<T>(std::atoll(s)); }
};

template <typename T>
struct parser<T,
              typename std::enable_if<std::is_floating_point<T>::value>::type> {
  static T parse(char *s) { return static_cast<T>(std::atof(s)); }
};
...
struct arg_maker {
  static const basic_argument make(const args_descriptor &argd,
    ...
    basic_argument arg(sizeof(T), nargs,
                       basic_argument::dscrpt_t(name, help, flag, required));
...
template <typename T, bool required> struct arg_maker<T, 0, required> {
  static const basic_argument make(const args_descriptor &argd,
                                   const std::string &name,
                                   const std::string &help,
                                   const std::string &flag) {
    basic_argument arg(1, 1,
                       basic_argument::dscrpt_t(name, help, flag, required));
    int idx = argd.find(name, flag);
...
template <typename T, int nargs> struct arg_validator<T, nargs, true> {
  static void validate(const args_descriptor &argd, const std::string &name,
                       const std::string &flag) {
    arg_validator<T, nargs, false>::validate(argd, name, flag);
    if (argd.find(name, flag) == -1)
      throw std::domain_error(std::string("Required argument [") + name +
                              "] not provided.");
  }
};
...
public:
  explicit argument_parser(std::string prog = "", std::string description = "",
                           std::string epilog = "")
      : argd_(__argc, __argv), prog_(prog), description_(description),
        epilog_(epilog) {
    init();
  }

  explicit argument_parser(int argc, char **argv, std::string prog = "",
                           std::string description = "",
                           std::string epilog = "")
      : argd_(argc, argv), prog_(prog), description_(description),
        epilog_(epilog) {
    init();
  }
...
private:
  ...
  void print_help() const {
    ...
    for (auto kv : prsd_args_) {
      std::string name =
          kv.second.description().required
    ...
    print_description(basic_argument::dscrpt_t(
        "--help", "Show this messsage and exit", "-h", false));
    for (auto kv : prsd_args_)
